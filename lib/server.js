		...
		r.on("onData", function() {
			router.get("/question/:id", function(req, res) {
				...
				if(question.alternatives !== undefined) {
					question.message = "You got your question! Now send me which alternative that is right (the key) as the answer via HTTP POST to the nextURL in JSON-format";
				} else {
			...
			router.route("/answer/:id").post(function(req, res) {
				...
				if(!req.body.hasOwnProperty("answer")) {
					res.status(400).send({"message" : "The JSON-input should have a property named 'answer' and Content-Type set to 'application/json'"});
					res.end();
					return;
				}
				else {
                    //use .toLowerCase() to not case about the casing in answers
					if(question.answer.toLowerCase() === req.body.answer.toString().toLowerCase()) {
			...
			console.log("Server listen on port " + app.get('port') +" in dev MODE");
			callback(server);
		});
